/**
 * Skill 批量生产模块（V2）
 * 结果展示收进工作台大抽屉，不再使用独立结果弹窗
 * 接口：GET /api/skills → 失败时回退 MOCK_SKILLS
 */
...
function simulateSkillResultReturn(task, resultIndex, delayMs) {
    setTimeout(() => {
        ...
        if (!isStoryboardResult(result) && result.url) {
            result.versions = [{ id: 'v0', url: result.url, label: 'Skill 原图', source: 'skill' }];
            result.activeVersionId = 'v0';
        }

        if (typeof updateSkillToolbarUI === 'function') updateSkillToolbarUI();
        if (typeof renderTable === 'function') renderTable();
        if (typeof renderWorkbenchIfOpen === 'function') {
            renderWorkbenchIfOpen(task.id);
        }
        if (typeof renderTable === 'function') renderTable();
    }, delayMs);
}

/** 对单条任务执行指定 Skill（工作台选用） */
function executeSkillOnTask(taskId, skillSlug) {
    const task = taskData.find(t => t.id === taskId);
    const skill = getSkillBySlug(skillSlug);
    if (!task || !skill) return false;

    if (typeof isSkillResultsLocked === 'function' && isSkillResultsLocked(task)) {
        showNotification('该任务已上传唯妙，无法再次执行 Skill', 'warning');
        return false;
    }
    ...
    showNotification(`已对任务执行「${skill.name}」`, 'success');
    if (typeof renderTable === 'function') renderTable();
    if (typeof updateSkillToolbarUI === 'function') updateSkillToolbarUI();
    if (typeof renderWorkbenchIfOpen === 'function') {
        renderWorkbenchIfOpen(task.id);
...
function getSkillDescription(skill) {
    const types = (skill.generationTypes || []).join('、');
    const channels = (skill.channels || []).join('、');
    if (isStoryboardSkill(skill)) {
        return `适用 ${channels} · 产出分镜表 Excel`;
    }
    return `适用 ${channels} · ${types} · 产出 ${skill.outputCount} 项`;
}
...
/** V2：操作列工作台入口由 workbench-v2.js getWorkbenchInlineHTML 提供 */
function getSkillInlineActionHTML() {
    return '';
}

function getSkillResultEntryHTML(task) {
    return typeof getWorkbenchInlineHTML === 'function' ? getWorkbenchInlineHTML(task) : '';
}

function openSkillResultModal(taskId) {
    if (typeof openWorkbenchDrawer === 'function') {
        openWorkbenchDrawer(taskId);
    }
}

function refreshSkillResultUI(taskId) {
    if (typeof renderWorkbenchIfOpen === 'function') {
        renderWorkbenchIfOpen(taskId);
    }
}
...
function handleSkillLocalUpload(event) {
    ...
    reader.onload = (e) => {
        ...
        if (result.versions) {
            result.versions.push({ id: `local_${Date.now()}`, url: result.url, label: '本地上传', source: 'local' });
            result.activeVersionId = result.versions[result.versions.length - 1].id;
        }
        refreshSkillResultUI(taskId);
        renderTable();
